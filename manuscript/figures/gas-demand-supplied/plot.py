...
High_Supplied = data.filter(variable='Gas|Demand|High|Supplied')
High_Not_Supplied = data.filter(variable='Gas|Demand|High|Not Supplied')

fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, sharex=False, figsize=(4,3))

High_Supplied.plot(ax=ax1, legend=False, color='scenario', fill_between=True)
High_Not_Supplied.plot(ax=ax2, legend=False, color='scenario', fill_between=True)
...
# draw_brace(ax1, (6, 16),'Decommissioning\n(10 out of 16)')
...
fig.savefig('demand.png', dpi=1000)
fig.savefig('demand.eps', format='eps')
