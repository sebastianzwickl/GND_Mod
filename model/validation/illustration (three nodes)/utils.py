...
def init_nodal_demand_at_mid_pressure(model, node, year, time):
    ...
    if _data.empty:
        ...
        _case = _data["Time-res."].item()
        _temp = _temp.loc[time - 1, _data["Time-res."].item()].item()
        return (_data["Yr.-dem."].item() * _temp) * (0.85) ** (year - 2025)
...
def add_decision_variables(model=None):
    ...
    model.var_gas_purchase = py.Var(
        model.set_year,
        domain=py.NonNegativeReals) 
    
    
    """DELIVERY BETWEEN TRANSMISSION AND HIGH-PRESSURE NETWORK LEVEL"""
    model.var_del_tra_high = py.Var(model.set_delivery_tra_hp, model.set_year, model.set_time_unit,
                                    domain=py.NonNegativeReals)
    model.var_del_high_mid = py.Var(model.set_delivery_hp_mp, model.set_year, model.set_time_unit,
                                    domain=py.NonNegativeReals)
...
def obj_value(model=None):
    ...
    return sum(
        ...
        * (model.var_capex[year] + model.var_opex[year] - model.var_rev[year] + model.var_gas_purchase[year])
